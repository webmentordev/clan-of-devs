...
<section class="h-screen bg-dark-light flex">
    ...
    <section class="w-full h-full p-2">
        <div class="flex flex-wrap gap-3">
            @forelse ($workspaces as $workspace)
                <div wire:click='assign_data("{{ $workspace->unique_id }}")' class="flex flex-col w-full max-w-50 bg-dark-light-100 rounded-xl overflow-hidden border border-white/5 cursor-pointer">
                    ...
                    <div class="flex flex-col justify-between flex-1 p-4 gap-3">
                        <div class="flex flex-col gap-3">
                            <div>
                                ...
                                <p class="text-txt-2 text-xs mt-1 leading-relaxed">{{ Str::limit($workspace->description, 50, '...') }}</p>
                            </div>
                        ...
                        <button wire:click.stop='join_workspace("{{ $workspace->unique_id }}")' class="w-full bg-main hover:bg-main/80 text-white text-sm font-semibold py-2.5 rounded-lg transition-opacity duration-150">
                            Join workspace
                        </button>
                    </div>
                </div>
    ...
    @if ($workspace_data)
        ...
        @livewire('components.workspace-info', ['data' => $workspace_data], key($selected_workspace_id))
    @endif
</section>